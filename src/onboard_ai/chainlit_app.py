"""
This module defines the Chainlit application entry point.

It handles the chat interface lifecycle, including:
- Session initialization (on_chat_start)
- User settings management (on_settings_update)
- Message handling and routing to AutoGen agents (on_message)

The application leverages GraphRAG for retrieval and AutoGen for orchestration.
"""
...
async def on_chat_start() -> None:
    """Initialize the chat session: render settings, create agents, check index."""
    try:
...
async def setup_agent(settings: dict) -> None:
    """Persist updated chat settings to the user session."""
    cl.user_session.set("Gen_type", settings["Gen_type"])
    cl.user_session.set("Audience", settings["Audience"])
    cl.user_session.set("Enable_web_search", settings["Enable_web_search"])
    cl.user_session.set("Enable_multi_agent", settings["Enable_multi_agent"])
...
@cl.on_message
async def run_conversation(message: cl.Message) -> None:
    """Handle an incoming user message: route through GroupChat agents."""
    root_dir = Path(".")
